<template>
    <div>
        ...
        <div class="container">
            <h2 class="mt-2">Печать наклеек</h2>
            <div class="row">
                <div class="col-md-4">
                    <input class="form-control" type="date" v-model="selectDate"/>
                </div>
                <div class="col-md-3" v-if="forPrint.length > 0">
            ...
            <div class="row" style="overflow-x: scroll">
                <div class="col-md-12">
                    <table class="table table-striped table-hover" width="100%">
                        ...
                        <tbody>
                        <tr v-for="(item,index) in activeDeliverys" :key="index">
                            <td>
...
export default {
    ...
    watch: {
        selectDate(e) {
            ...
            fetch("/maxoptralist?token=" + self.token + "&date=" + date)
                .then(response => {
                    return response.json();
                })
                .then(text => {
                    let orders = JSON.parse(
                        convert.xml2json(text.orders, {compact: true})
                    );
                    self.orders = orders.apiResponse.OrdersWithZoneResponse.orders.order;
                    let deliverys = JSON.parse(
                        convert.xml2json(text.deliverys, {
                            compact: true
                        })
                    );
                    return deliverys.apiResponse.scheduleResponse.vehicles.vehicle;
                })
                .then(cars => {
                    this.currentDel(cars);
                })
                .catch(alert => {
                    console.log(alert);
                });
    ...
    methods: {
        goToPrint() {
            ...
            this.forPrint.forEach(element => {
                if (element.packages != null) {
                    let nE = element.packages.split(" ");
                    nE.forEach(elements => {
                        if (elements != null) {
                            let neE = elements.split("");
                            for (let index = 0; index < neE[1]; index++) {
                                const canvas = createCanvas(0, 0);
                                JsBarcode(canvas, neE[0] + element.orderReference);
                                docInfo.content.push(
                                    ...
                                    {
                                        fontSize: 150,
                                        text: element.driverNumber + " " + element.driverName,
                                        pageBreak: "after",
                                        alignment: "center"
                                    }
                                );
        ...
        select() {
            this.forPrint = [];
            if (!this.selectAll) {
                for (let i in this.activeDeliverys) {
                    this.forPrint.push(this.activeDeliverys[i]);
                }
            }
        },
        getClientName(id) {
