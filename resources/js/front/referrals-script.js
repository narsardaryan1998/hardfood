...
var app = new Vue({
    ...
    data: {
        ...
        userData: {
            ...
            userActiveReferralsCount: 0,
            promoCodeMoreBonusesByPercent: false,
            promoCodeMoreBonusesPercent: 0,
        },
        userRes: false,
        menuRes: false,
        referralTypeIsWeek: false,
    },
    created() {
        this.getActiveUserReferralsCount();
        this.getUser();
        this.getMenu();
    ...
    methods: {
        ...
        submitGiftAWeekOrder() {
            this.orderLoading = true;
            axios({
                method: "post",
                url: "/api/add-order/order-to-crm-gift-a-week"
            }).then(response => {
        ...
        copyClipboardPromoCode() {
            var promocode = document.getElementById("promoCodeValue").innerText;
            navigator.clipboard.writeText(promocode);
            this.modalAction('success-copied-popap', false);
        },
        copyReferralLink() {
            var userReferralLink = document.getElementById("userReferralLinkValue").innerText;
        ...
        getUser() {
            axios({
                ...
                if (response.data) {
                    ...
                    this.$set(this.userData, 'email', response.data.email ? response.data.email : 0)
                    this.$set(this.userData, 'quantityOfGiftAWeek', response.data.quantityOfGiftAWeek ? response.data.quantityOfGiftAWeek : 0)
                }
                this.userRes = true;
            });
        },
        getActiveUserReferralsCount() {
            axios({
                method: "get",
                url: "/api/get-user-active-referrals-count",
            }).then(response => {
                this.$set(this.userData, 'userActiveReferralsCount', response.data)
            });
        },
        logout() {
            axios({
