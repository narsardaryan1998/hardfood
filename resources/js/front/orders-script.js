...
var app = new Vue({
    ...
    data: {
        ...
        idOfDeletableAddress: false,
        openedDeliveryActionIndex: false,
        openedOrderIndex: false,
        updatedDeliveryIndex: false,
        selectedAddress: '',
        currentDay: 1,
        excludes: {
            dish: [],
        ...
        changeableDeliveryId: 0,
        changeableDeliveryAction: '',
    ...
    methods: {
        ...
        openDeliveryAction(index) {
            if (this.openedDeliveryActionIndex === index) {
                this.openedDeliveryActionIndex = false
                this.updatedDeliveryIndex = false
                this.changeableDeliveryAction = ''
            } else {
                this.openedDeliveryActionIndex = index;
                this.updatedDeliveryIndex = index;
            }
        },
        addAddressForDelivery(deliveryId) {
            var deliveryDate = this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].delivery_date;
            var today = moment().tz('Europe/Moscow').format("YYYY-MM-DD")
            if (deliveryDate === today && moment().tz('Europe/Moscow').format("HH:mm:ss") > '12:00:00') {
                this.modalAction('cant-do-changes', false)
            } else {
                axios.post('/api/add-new-address-for-delivery', {
                    deliveryId: deliveryId,
                    newAddress: this.newAddress,
                    amocrmId: this.userOrdersCurrent[this.openedOrderIndex].orderNumber
                }).then(response => {
                    if (response.data.success) {
                        this.modalAction('address-success-added', false)
                        this.getUser();
                        this.getUserOrders();
                    }
                })
        ...
        updateDeliveryAddress(selectedAddress, deliveryId) {
            var deliveryDate = this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].delivery_date;
            var today = moment.tz('Europe/Moscow').format("YYYY-MM-DD")
            if (deliveryDate === today && moment.tz('Europe/Moscow').format("HH:mm:ss") > '12:00:00') {
                this.modalAction('cant-do-changes', false)
            } else {
                this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].selected_address = selectedAddress;
                this.modalAction('success-updated', false)
                if (selectedAddress === 1) {
                    var address = this.userData.addressStreet ? this.userData.addressStreet : '';
                    var porch = this.userData.addressPorch ? this.userData.addressPorch : 'не указан';
                    var intercom = this.userData.addressIntercom ? this.userData.addressIntercom : 'не указан';
                    var flat = this.userData.addressFlat ? this.userData['addressFlat'] : 'не указан';
                    var floor = this.userData.addressFloor ? this.userData.addressFloor : 'не указан';
                    var commentForOrder = this.userData.commentForOrder ? this.userData.commentForOrder : 'не указан';
                } else if (selectedAddress === 2) {
                    var address = this.userData.addressStreet2 ? this.userData.addressStreet2 : '';
                    var porch = this.userData.addressPorch2 ? this.userData.addressPorch2 : 'не указан';
                    var intercom = this.userData.addressIntercom2 ? this.userData.addressIntercom2 : 'не указан';
                    var flat = this.userData.addressFlat2 ? this.userData.addressFlat2 : 'не указан';
                    var floor = this.userData.addressFloor2 ? this.userData.addressFloor2 : 'не указан';
                    var commentForOrder = this.userData.commentForOrder2 ? this.userData.commentForOrder2 : 'не указан';
                }
                let deliveryAddress = address + ', Подьезд: ' + porch + ', Домофон: ' + intercom + ', Этаж: ' + floor + ', Квартира: ' + flat + ', Комментарии: ' + commentForOrder;
                this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].address = deliveryAddress;
                axios.post('/api/update-delivery-address', {
                    selectedAddress,
                    deliveryId,
                    deliveryAddress,
                    amocrmId: this.userOrdersCurrent[this.openedOrderIndex].orderNumber
        ...
        updateAddress() {
            ...
            var today = moment().tz('Europe/Moscow').format("YYYY-MM-DD")
            if (deliveryDate === today && moment().tz('Europe/Moscow').format("HH:mm:ss") > '12:00:00') {
                this.modalAction('cant-do-changes', 'chenge-addres')
            } else {
                if (this.updatedAddressIndex) {
                    this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].selected_address = this.updatedAddressIndex;
                    axios({
                        method: "post",
                        url: "/api/address/update",
                        params: {
                            params: this.updatedAddress,
                            deliveryId: this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].id,
                            index: this.updatedAddressIndex,
                            amocrmId: this.userOrdersCurrent[this.openedOrderIndex].orderNumber
                        }
                    })
                    ...
                    if (this.updatedAddressIndex === 1) {
                        this.userData = Object.assign({}, this.userData, {
                            addressFlat: this.updatedAddress.addressFlat,
                            addressStreet: this.updatedAddress.addressStreet,
                            addressPorch: this.updatedAddress.addressPorch,
                            addressFloor: this.updatedAddress.addressFloor,
                            addressIntercom: this.updatedAddress.addressIntercom,
                        })
                    } else {
                        this.userData = Object.assign({}, this.userData, {
                            addressFlat2: this.updatedAddress.addressFlat,
                            addressStreet2: this.updatedAddress.addressStreet,
                            addressPorch2: this.updatedAddress.addressPorch,
                            addressFloor2: this.updatedAddress.addressFloor,
                            addressIntercom2: this.updatedAddress.addressIntercom,
                        })
                    }
                    var address = this.updatedAddress.addressStreet ? this.updatedAddress.addressStreet : '';
                    var porch = this.updatedAddress.addressPorch ? this.updatedAddress.addressPorch : 'не указан';
                    var intercom = this.updatedAddress.addressIntercom ? this.updatedAddress.addressIntercom : 'не указан';
                    var flat = this.updatedAddress.addressFlat ? this.userData['addressFlat'] : 'не указан';
                    var floor = this.updatedAddress.addressFloor ? this.updatedAddress.addressFloor : 'не указан';
                    var commentForOrder = this.updatedAddress.commentForOrder ? this.updatedAddress.commentForOrder : 'не указан';
                    this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].address = address + ', Подьезд: ' + porch + ', Домофон: ' + intercom + ', Этаж: ' + floor + ', Квартира: ' + flat + ', Комментарии: ' + commentForOrder;
                    this.updatedAddress = Object.assign({}, this.updatedAddress, {
                        addressFlat: '',
                        addressStreet: '',
                        addressPorch: '',
                        addressFloor: '',
                    ...
                    this.updatedAddressIndex = false
                    this.modalAction('success-updated', 'chenge-addres')
                }
            }
        },
        deleteAddress() {
            if (this.idOfDeletableAddress) {
                axios({
                    method: "post",
                    url: "/api/address/delete",
                    params: {
                        id: this.idOfDeletableAddress,
                    }
                }).then(response => {
                    if (response.data.success) {
                        if (this.idOfDeletableAddress === 1) {
                            this.userData = Object.assign({}, this.userData, {
                                addressFlat: '',
                                addressStreet: '',
                                addressPorch: '',
                        ...
                        this.idOfDeletableAddress = false;
                    }
                })
                this.modalAction('address-success-deleted', 'delete-question-popap')
            }
        },
        openUpdateAddressModal(index) {
            ...
            if (index == 1) {
                this.updatedAddress = Object.assign({}, this.updatedAddress, {
                    addressFlat: this.userData.addressFlat,
                    addressStreet: this.userData.addressStreet,
                    addressPorch: this.userData.addressPorch,
                    addressFloor: this.userData.addressFloor,
                    addressIntercom: this.userData.addressIntercom,
                })
            } else {
                this.updatedAddress = Object.assign({}, this.updatedAddress, {
                    addressFlat: this.userData.addressFlat2,
                    addressStreet: this.userData.addressStreet2,
                    addressPorch: this.userData.addressPorch2,
                    addressFloor: this.userData.addressFloor2,
                    addressIntercom: this.userData.addressIntercom2,
                })
            }
        },
        ...
        getUserOrders() {
            axios({
                ...
                if (response.data) {
                    this.userOrdersCurrent = response.data.currentOrders;
                    this.userOrdersOld = response.data.oldOrders;
                    this.allMenuForDelivery = response.data.actualMenu;
                    this.ordersRes = true;
                }
            });
        },
        ...
        submitOrder() {
            ...
            if (!this.userData.name) {
                ...
                axios.post('/api/add-order/order-to-crm', {
                    ...
                    if (response.data.success) {
                        ym(44931988, 'reachGoal', 'orderdone')
                        gtag('event', 'Оформленный заказ (событие)', {
        ...
        getDeliveryDays() {
            ...
            var deliveryDate = this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex].delivery_date;
            let addDays = 2;
            if (moment(deliveryDate).format("D") === 0) {
                addDays = 3;
            }
            var nextDeliveryDate = moment(deliveryDate).add(addDays, 'days').format("YYYY-MM-DD")
            for (let i = 0; i <= addDays; i++) {
                let dayOfWeek = moment(deliveryDate).add(i, 'days').day();
                let date = moment(deliveryDate).add(i, 'days').format("D");
                this.deliveryDateArray.push(
                    {
                        currentWeekDayNumber: dayOfWeek,
                        currentWeekDateNumber: date,
                    }
                );
                if (!i && this.deliveryCurrentDay === false) {
                    this.deliveryCurrentDay = dayOfWeek;
                }
            }
            if (!this.isDesktop && !document.getElementsByClassName('delivery_menu_menu_mobile_' + this.openedOrderIndex + '_' + this.updatedDeliveryIndex)[0].classList.contains('slick-initialized')) {
                this.installSlick()
            }
            this.getDeliveryMenu(deliveryDate, nextDeliveryDate, this.deliveryCurrentDay);
        },
        installSlick() {
            $('.delivery_menu_menu_mobile_' + this.openedOrderIndex + '_' + this.updatedDeliveryIndex).slick({
                arrows: true,
                slidesToShow: 4.2,
                speed: 900,
                Infinite: false,
        ...
        getDeliveryMenu(deliveryDate, nextDeliveryDate, day) {
            this.menuForDelivery = [];
            if (!day) {
                day = 7;
            }
            let delivery = this.userOrdersCurrent[this.openedOrderIndex].deliveries[this.updatedDeliveryIndex];
            var maxRacion = 's';
            if ((delivery.l7 && delivery.l7 != '0') || (delivery.l5 && delivery.l5 != '0')) {
                maxRacion = 'l';
            } else if ((delivery.m7 && delivery.m7 != '0') || (delivery.m5 && delivery.m5 != '0')) {
                maxRacion = 'm';
            }

            for (let i = 0; i < this.allMenuForDelivery.length; i++) {
                if (this.menuForDelivery.length === 6) {
                    break;
                }
                const deliveryDateMoment = moment(deliveryDate);
                const nextDeliveryDateMoment = moment(nextDeliveryDate);
                const from_date = deliveryDateMoment.startOf('week').format("YYYY-MM-DD");
                const to_date = nextDeliveryDateMoment.endOf('week').format("YYYY-MM-DD");
                if (this.allMenuForDelivery[i].weekDay == day && this.allMenuForDelivery[i].startDate >= from_date && to_date > this.allMenuForDelivery[i].startDate) {
                    this.menuForDelivery.push(this.allMenuForDelivery[i]);
                }
            }
            if (this.menuForDelivery.length === 6) {
                this.menuForDelivery.sort((a, b) => (a.priority > b.priority) ? 1 : ((b.priority > a.priority) ? -1 : 0));
                if (maxRacion === 's') {
                    this.menuForDelivery.splice(-2);
                } else if (maxRacion === 'm') {
                    this.menuForDelivery.splice(-1);
                }
            }
            if (this.isDesktop) {
                $('.delivery_menu_menu').slick('slickRemove', null, null, true);
                this.menuForDelivery.forEach((value, index) => {
                    index++;
                ...
                $('.delivery_menu_menu_mobile_' + this.openedOrderIndex + '_' + this.updatedDeliveryIndex).slick('slickRemove', null, null, true);
                this.menuForDelivery.forEach((value, index) => {
                    index++;
    ...
    watch: {
        ...
        updatedDeliveryIndex: function (val) {
            if (val !== false && this.openedOrderIndex !== false) {
                this.deliveryCurrentDay = false;
                this.getDeliveryDays()
            }
        },
        menuRes: function () {
            if (this.menuRes && this.ordersRes && this.userRes) {
...
$('._burger-btn').click(function () {
    ...
    if ($('.header__menu').hasClass('_scroll') == false) {
        $('.header__menu').toggleClass('_scroll');
    }
});
